...
async def start(message: types.Message, state: FSMContext):
    ...
    with PostgresSession.begin() as session:
        ...
        if user := session.get(CommonUser, user_id):
            ...
            await message.answer(
                'В этом чат-боте ты сможешь получить баллы за участие в мероприятиях и другие активности. '
                'Участники, заработавшие больше всех баллы, получат призы от Госкорпорации "Росатом"\n\n'
